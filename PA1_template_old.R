...
# Create a column for the value of interval within day (0 to 55)
...
# For this part we ignore (ignore) remove the missing values in the dataset.
...
print("5-min interval which on average across all the days in the dataset,
       contains the maximum number of steps:")
print("Interval:")
...
print("Use the mean value value computed previously for that this interval")
...
for (n in 1:nrow(activity)) {
  if (is.na(activity$steps[n])) {
    activity$steps[n] <- interval_means[interval_means$Group.1 == activity$day_interval[n], ]$x
  }
}
